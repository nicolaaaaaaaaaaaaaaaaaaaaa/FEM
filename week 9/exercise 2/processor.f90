...
!
    subroutine output
        ...
        write (10, '("number      1-direction     2-direction     3-direction      sigma 1         sigma 2        direction" )')
        do e = 1, ne
            select case( element(e)%id )
            case( 1 )
                write (10, '(1x, i6, 1x, f15.9, 34x, f15.9)') e, strain(e, 1), stress(e, 1)
            case( 2 )
                write (10, '(1x, i6, 3(1x, f15.9), 1x, 3(1x, f15.9))')e,(strain(e, i),i = 1,3),(principal_stresses(e, i),i = 1,3)
            end select
        end do
...
!
    subroutine stopwatch(oper)
        ...
        case ('STOP', 'stop')
            ...
            write (6, '(8x, 1a, 1f18.10)') 'elapsed wall clock time:', finish_time - start_time
        case default
