...
for i in range(len(lines)-1):
    ...
    pattern16 = re.compile(r'0\.[0-9][0-9]D$')
    pattern17 = re.compile(r'^[0-9]')
    if pattern16.search(line) and pattern17.search(next_line):
        digits = next_line[0]
        line = line + digits
        next_line = next_line.strip(digits)


    if next_line.startswith(name_matrix):
