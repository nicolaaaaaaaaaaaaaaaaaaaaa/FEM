...
!
    subroutine shell41_ss(xe, de, young, youngy, theta, shear, nu, thk, estress, estrain, eprincipal_stresses, esigmavm)
        ...
        real(wp) :: aa, bb, x, y, bmat(3,12), cmat(3,3), cmat_plane(3, 3), cmat_ortho(3, 3), T(3,3) , c, s, EE, EEY, GXY
        real(wp) :: alpha, sigma1, sigma2, sinpsi, cospsi, psi
        ...
        Bmat(1,1) = 0.3D1 / 0.4D1 * x * (bb - y) / aa ** 3 / bb
        Bmat(1,2) = -(bb - y) * (-0.3D1 * x + aa) / aa ** 2 / bb / 0.4D1
        Bmat(1,4) = -0.3D1 / 0.4D1 * x * (bb - y) / aa ** 3 / bb
        Bmat(1,5) = (bb - y) * (0.3D1 * x + aa) / aa ** 2 / bb / 0.4D1
        Bmat(1,7) = -0.3D1 / 0.4D1 * x * (bb + y) / aa ** 3 / bb
        Bmat(1,8) = (0.3D1 * x + aa) * (bb + y) / aa ** 2 / bb / 0.4D1
        Bmat(1,10) = 0.3D1 / 0.4D1 * x * (bb + y) / aa ** 3 / bb
        Bmat(1,11) = -(bb + y) * (-0.3D1 * x + aa) / aa ** 2 / bb / 0.4D1
        Bmat(2,1) = 0.3D1 / 0.4D1 * y * (aa - x) / aa / bb ** 3
        Bmat(2,3) = -(-0.3D1 * y + bb) * (aa - x) / aa / bb ** 2 / 0.4D1
        Bmat(2,4) = 0.3D1 / 0.4D1 * y * (aa + x) / aa / bb ** 3
        Bmat(2,6) = -(-0.3D1 * y + bb) * (aa + x) / aa / bb ** 2 / 0.4D1
        Bmat(2,7) = -0.3D1 / 0.4D1 * y * (aa + x) / aa / bb ** 3
        Bmat(2,9) = (0.3D1 * y + bb) * (aa + x) / aa / bb ** 2 / 0.4D1
        Bmat(2,10) = -0.3D1 / 0.4D1 * y * (aa - x) / aa / bb ** 3
        Bmat(2,12) = (0.3D1 * y + bb) * (aa - x) / aa / bb ** 2 / 0.4D1
        Bmat(3,1) = 0.1D1 / bb / aa - 0.3D1 / 0.4D1 * x ** 2 / aa ** 3 / bb&
         - 0.3D1 / 0.4D1 * y ** 2 / bb ** 3 / aa
        Bmat(3,2) = (aa ** 2 + 0.2D1 * x * aa - 0.3D1 * x ** 2) / aa ** 2&
        / bb / 0.4D1
        Bmat(3,3) = (bb ** 2 + 0.2D1 * y * bb - 0.3D1 * y ** 2) / aa / bb&
        ** 2 / 0.4D1
        Bmat(3,4) = -0.1D1 / bb / aa + 0.3D1 / 0.4D1 * x ** 2 / aa ** 3 /&
        bb + 0.3D1 / 0.4D1 * y ** 2 / bb ** 3 / aa
        Bmat(3,5) = (aa ** 2 - 0.2D1 * x * aa - 0.3D1 * x ** 2) / aa ** 2&
        / bb / 0.4D1
        Bmat(3,6) = (-bb ** 2 - 0.2D1 * y * bb + 0.3D1 * y ** 2) / aa / bb&
        ** 2 / 0.4D1
        Bmat(3,7) = 0.1D1 / bb / aa - 0.3D1 / 0.4D1 * x ** 2 / aa ** 3 / bb&
         - 0.3D1 / 0.4D1 * y ** 2 / bb ** 3 / aa
        Bmat(3,8) = (-aa ** 2 + 0.2D1 * x * aa + 0.3D1 * x ** 2) / aa ** 2&
        / bb / 0.4D1
        Bmat(3,9) = (-bb ** 2 + 0.2D1 * y * bb + 0.3D1 * y ** 2) / aa / bb&
        ** 2 / 0.4D1
        Bmat(3,10) = -0.1D1 / bb / aa + 0.3D1 / 0.4D1 * x ** 2 / aa ** 3 /&
        bb + 0.3D1 / 0.4D1 * y ** 2 / bb ** 3 / aa
        Bmat(3,11) = (-aa ** 2 - 0.2D1 * x * aa + 0.3D1 * x ** 2) / aa **&
        2 / bb / 0.4D1
        Bmat(3,12) = (bb ** 2 - 0.2D1 * y * bb - 0.3D1 * y ** 2) / aa / bb&
        ** 2 / 0.4D1


        ! compute element strain
        estrain = -thk/2*matmul(bmat, de)
        ...
        cmat_plane(3,1) = 0
        cmat_plane(3,2) = 0
        cmat_plane(3,3) = (1-nu)/2
        cmat_plane = young/(1-nu**2)*cmat_plane


        alpha = 1 - (nu**2 * youngy)/ young

        cmat_ortho(1,1) = 1
        cmat_ortho(1,2) = nu*youngy/young
        cmat_ortho(1,3) = 0
        cmat_ortho(2,1) = nu*youngy/young
        cmat_ortho(2,2) = youngy/young
        cmat_ortho(2,3) = 0
        cmat_ortho(3,1) = 0
        cmat_ortho(3,2) = 0
        cmat_ortho(3,3) = alpha*shear/young
        cmat_ortho = young/alpha*cmat_ortho
        ...
        cmat = matmul(matmul(transpose(T), cmat_ortho), T)



        ! Compute element stress
        estress = matmul(cmat, estrain)
        !estress = matmul(cmat_plane, estrain)
